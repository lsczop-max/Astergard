...
class D57RegionalKnowledgeBankTests(unittest.TestCase):
    ...
    def test_all_known_regions_have_banks_with_full_category_coverage(self) -> None:
        self.assertEqual(len(REGIONAL_KNOWLEDGE_BANKS), 6)
    ...
    def test_surface_selection_is_deterministic_and_region_specific(self) -> None:
        ...
        first = pick_surface_phrases(bank_for_region("polnocny-las"), location_type="ścieżka", terrain="leśny", state=state, rng=Random(7))
        second = pick_surface_phrases(bank_for_region("polnocny-las"), location_type="ścieżka", terrain="leśny", state=state, rng=Random(7))
        third = pick_surface_phrases(bank_for_region("nadrzeczne-mokradla"), location_type="bagno", terrain="bagienny", state=state, rng=Random(7))
    ...
    def test_sensory_selection_stays_region_tied(self) -> None:
        ...
        forest_senses = pick_sensory_phrases(bank_for_region("polnocny-las"), location_type="ścieżka", terrain="leśny", state=state, rng=Random(2))
        swamp_senses = pick_sensory_phrases(bank_for_region("nadrzeczne-mokradla"), location_type="bagno", terrain="bagienny", state=state, rng=Random(2))
    ...
    def test_generation_audit_can_sample_a_region(self) -> None:
        report = audit_region_generation(self.generator, self.world, "polnocny-las", sample_size=12)
        self.assertEqual(report.region_id, "polnocny-las")
    ...
    def test_style_guide_and_text_audit_are_buildable(self) -> None:
        bank = bank_for_region("centrum")
        sample_texts = [self.generator.generate(room_id).long_description for room_id in (0, 1, 14, 21)]
        audit = audit_region_texts("centrum", sample_texts, bank)
        guide = build_style_guide("centrum", bank, audit)
        self.assertEqual(guide.region_id, "centrum")
