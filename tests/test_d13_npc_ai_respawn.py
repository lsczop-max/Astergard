...
class D13NPCAITests(unittest.TestCase):
    ...
    def test_respawn_waits_for_delay_and_respects_room_population_limit(self) -> None:
        ...
        first_respawn_time = float(str(world.respawn_queue[0]["time"]))
        early = npcs.respawn_tick(now=first_respawn_time + 9)
        self.assertEqual(early, [])
        self.assertEqual(len(world.respawn_queue), 1)
        second_respawn_time = float(str(world.respawn_queue[0]["time"]))
        late = npcs.respawn_tick(now=second_respawn_time + 11)
