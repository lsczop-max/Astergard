...
class D34WorldAreaExpansionTests(unittest.TestCase):
    ...
    def test_authored_rooms_keep_world_graph_size_and_have_inspectables(self) -> None:
        ...
        self.assertGreaterEqual(len(authored), 60)
    ...
    def test_player_can_travel_into_expanded_area_and_inspect_generic_detail(self) -> None:
        async def run() -> None:
            with TestGameHarness() as harness:
                ...
                self.assertIn("Kramy Płócienników", transcript.output)
                self.assertIn("Możesz obejrzeć", transcript.output)
                detail = await harness.execute(char, "obejrzyj ludzi")
                self.assertIn("Miejscowi patrzą", detail.output)
