...
class CombatManager:
    ...
    def initiative_score(self, character: Character) -> int:
        ...
        wound_penalty = character.wounds.get("prawa_noga", 0) + character.wounds.get("lewa_noga", 0)
        ...
        return character.stats.zrecznosc + weapon_modifier + style.initiative_modifier + self.rng.randint(1, self.rules.initiative_roll_sides) - wound_penalty - stamina_penalty
    ...
    def attack(self, attacker: Character, defender: Character) -> CombatResult:
        ...
        weapon_skill = self._weapon_skill_name(weapon.damage_type if weapon else None)
        atk_skill = attacker.skills.values.get(weapon_skill, {"level": 1})["level"]
        dodge = defender.skills.values.get("uniki", {"level": 1})["level"]
        ...
        hit_score = attacker.stats.zrecznosc + atk_skill + weapon_reach + attacker_style.attack_modifier + self.rng.randint(1, self.rules.attack_roll_sides)
        if attacker.stats.kondycja == 0:
            hit_score //= self.rules.exhausted_hit_divisor
        dodge_score = defender.stats.zrecznosc + dodge + defender_style.defense_modifier + self.rng.randint(1, self.rules.attack_roll_sides)
        if hit_score <= dodge_score:
            ...
            return CombatResult(False, message, observer_message=message, style_used=attacker_style.name)
        ...
        effective = weapon_damage + attacker_style.damage_modifier - protection
        ...
        if effective <= 0:
            ...
            return CombatResult(True, message, body_part=part, effective_damage=effective, observer_message=message, style_used=attacker_style.name)
        ...
        return CombatResult(True, msg, dead, body_part=part, effective_damage=effective, observer_message=msg, style_used=attacker_style.name)

    def _active_defense(self, defender: Character, hit_score: int) -> CombatResult | None:
        defender_style = self.rules.style(defender.combat_style)
        shield = defender.shield()
        if shield is not None:
            block_score = defender.stats.zrecznosc + defender_style.defense_modifier + shield.shield_block + self.rng.randint(1, self.rules.active_defense_roll_sides)
        ...
        if weapon is not None and weapon.durability > 0:
            parry_level = defender.skills.values.get("parowanie", {"level": 1})["level"]
            parry_score = defender.stats.zrecznosc + defender_style.defense_modifier + parry_level + weapon.parry_bonus + self.rng.randint(1, self.rules.active_defense_roll_sides)
    ...
    def _weapon_skill_name(self, damage_type: str | None) -> str:
        if damage_type == "obuchowa":
            return "bron_obuchowa"
        if damage_type == "kluta":
            return "wlocznie"
        return "bron_cieta"
